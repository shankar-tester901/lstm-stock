...
Original file is located at
    https://colab.research.google.com/drive/1uUvSrZcof4tH2cnN6XoBt2mdy93qhWzA
"""
...
for ui in range(num_unrollings):
    train_inputs.append(
        tf.placeholder(tf.float32, shape=[batch_size, D], name="train_inputs_%d" % ui)
    )
    train_outputs.append(
        tf.placeholder(tf.float32, shape=[batch_size, 1], name="train_outputs_%d" % ui)
    )
...
w = tf.get_variable(
    "w", shape=[num_nodes[-1], 1], initializer=tf.contrib.layers.xavier_initializer()
)
...
with tf.control_dependencies(
    [tf.assign(sample_c[li], sample_state[li][0]) for li in range(n_layers)]
...
start_alpha = 0.25
alpha = np.arange(
    start_alpha, 1.1, (1.0 - start_alpha) / len(predictions_over_time[::3])
)
